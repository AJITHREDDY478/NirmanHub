    ...
    <nav className={`sticky top-0 z-50 transition-all duration-300 px-4 md:px-6 ${isShrunk ? 'py-2 md:py-3' : 'py-3 md:py-4'} bg-white/70 backdrop-blur-xl border-b border-slate-200/70 shadow-[0_10px_30px_rgba(15,23,42,0.08)]`}>
      ...
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        ...
        <Link to="/" className="flex items-center gap-3 group">
          <img src={brandLogo} alt="AR Print Lab" className="h-9 md:h-11 w-auto transform group-hover:scale-[1.02] transition-transform duration-300" />
        </Link>

        {/* Nav Links (Desktop) */}
        <div className="hidden md:flex items-center gap-3">
          <Link to="/" className="px-4 py-2 rounded-full text-slate-700 hover:text-blue-700 hover:bg-cyan-100/60 transition-all font-semibold">Home</Link>
          <Link to="/categories" className="px-4 py-2 rounded-full text-slate-700 hover:text-blue-700 hover:bg-cyan-100/60 transition-all font-semibold">Categories</Link>
          ...
          <div onMouseEnter={() => setShowGiftsMenu(true)} onMouseLeave={() => setShowGiftsMenu(false)} className="relative">
            ...
            <div className={`absolute left-1/2 -translate-x-1/2 top-full mt-4 w-[640px] bg-white rounded-3xl shadow-[0_30px_60px_rgba(15,23,42,0.18)] border border-slate-200 transition-all duration-300 ${showGiftsMenu ? 'opacity-100 visible' : 'opacity-0 invisible'}`}>
              <div className="p-6 grid grid-cols-2 gap-8">
                <div>
                  <h3 className="font-display font-bold text-slate-800 mb-4 flex items-center gap-2"><span className="text-xl">🎁</span>Shop By Occasion</h3>
                ...
                <div>
                  <h3 className="font-display font-bold text-slate-800 mb-4 flex items-center gap-2"><span className="text-xl">🎀</span>Gift Shop</h3>
                  <div className="space-y-2">
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          <div className="relative group">
            <button onClick={onOpenSearch} className="p-2 rounded-full bg-white/80 border border-slate-200/70 hover:bg-cyan-50 transition-all duration-300 ripple-effect touch-manipulation shadow-sm">
          ...
          <div className="relative account-menu-container">
            <button onClick={() => setShowAccountMenu(!showAccountMenu)} className="p-2 rounded-full bg-white/80 border border-slate-200/70 hover:bg-cyan-50 transition-all duration-300 ripple-effect touch-manipulation group shadow-sm">
              <svg className="w-5 h-5 sm:w-6 sm:h-6 text-slate-600 group-hover:text-cyan-600 transform group-hover:scale-110 transition-all" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
              </svg>
            </button>
            
            <div className={`absolute right-0 top-full mt-2 w-64 bg-white rounded-2xl shadow-[0_25px_50px_rgba(15,23,42,0.18)] border border-slate-200 transition-all duration-300 ${showAccountMenu ? 'opacity-100 visible scale-100' : 'opacity-0 invisible scale-95'}`}>
              <div className="p-4 border-b border-slate-200">
                {user ? (
                  <>
                    ...
                    <button onClick={handleSignOut} className="w-full px-4 py-2 bg-gradient-to-r from-blue-700 to-cyan-500 text-white text-sm font-semibold rounded-lg hover:shadow-lg hover:shadow-cyan-200/60 transition-all">Sign Out</button>
                  </>
                ) : (
                  <>
                    <p className="text-sm text-slate-600 mb-3">Welcome back!</p>
                    <div className="flex gap-2">
                      <button onClick={() => { onShowAuth('login'); setShowAccountMenu(false); }} className="flex-1 px-4 py-2 bg-gradient-to-r from-blue-700 to-cyan-500 text-white text-sm font-semibold rounded-lg hover:shadow-lg hover:shadow-cyan-200/60 transition-all">Login</button>
                      <button onClick={() => { onShowAuth('signup'); setShowAccountMenu(false); }} className="flex-1 px-4 py-2 border-2 border-slate-300 text-slate-700 text-sm font-semibold rounded-lg hover:border-cyan-400 transition-all">Sign Up</button>
                    </div>
                  </>
                )}
              </div>
              <div className="p-2">
                {user && (
                  <Link to="/orders" onClick={() => { setShowAccountMenu(false); setMobileMenuOpen(false); }} className="flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-cyan-50 transition-colors group">
                ...
                <Link to="/wishlist" onClick={() => { setShowAccountMenu(false); setMobileMenuOpen(false); }} className="flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-cyan-50 transition-colors group">
                  <svg className="w-5 h-5 text-slate-600 group-hover:text-cyan-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                {user && isAdminEmail(user.email) && (
                  <Link to="/address" onClick={() => { setShowAccountMenu(false); setMobileMenuOpen(false); }} className="flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-cyan-50 transition-colors group">
                    <svg className="w-5 h-5 text-slate-600 group-hover:text-cyan-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                {user && isAdminEmail(user.email) && (
                  <Link to="/products/upload" onClick={() => { setShowAccountMenu(false); setMobileMenuOpen(false); }} className="flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-cyan-50 transition-colors group">
                    <svg className="w-5 h-5 text-slate-600 group-hover:text-cyan-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <button onClick={() => setMobileMenuOpen(!mobileMenuOpen)} className="md:hidden p-2 rounded-full bg-white/80 border border-slate-200/70 hover:bg-cyan-50 transition-all touch-manipulation mobile-menu-button shadow-sm">
      ...
      <div className={`md:hidden absolute left-0 right-0 top-full transition-all duration-300 mobile-menu-container ${mobileMenuOpen ? 'opacity-100 visible' : 'opacity-0 invisible'}`}>
        <div className="mx-4 my-3 rounded-2xl bg-white/95 backdrop-blur-xl shadow-[0_25px_50px_rgba(15,23,42,0.18)] border border-slate-200/60 px-4 py-4 space-y-1 text-center">
          <Link to="/" onClick={() => setMobileMenuOpen(false)} className="block px-4 py-3 text-slate-700 hover:bg-cyan-50 rounded-xl transition-colors font-semibold">Home</Link>
          <Link to="/categories" onClick={() => setMobileMenuOpen(false)} className="block px-4 py-3 text-slate-700 hover:bg-cyan-50 rounded-xl transition-colors font-semibold">Categories</Link>
          {user && isAdminEmail(user.email) && (
            <Link to="/lithophane" onClick={() => setMobileMenuOpen(false)} className="block px-4 py-3 text-slate-700 hover:bg-cyan-50 rounded-xl transition-colors font-semibold flex items-center justify-center gap-2">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
              Lithophane
            </Link>
          )}
          <Link to="/about" onClick={() => setMobileMenuOpen(false)} className="block px-4 py-3 text-slate-700 hover:bg-cyan-50 rounded-xl transition-colors font-semibold">About</Link>
          <Link to="/contact" onClick={() => setMobileMenuOpen(false)} className="block px-4 py-3 text-slate-700 hover:bg-cyan-50 rounded-xl transition-colors font-semibold">Contact</Link>
        </div>
      </div>
    </nav>
  );
